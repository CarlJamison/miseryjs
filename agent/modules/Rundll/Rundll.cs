...
namespace Rundll
{
    ...
    public class Program
    {
        ...
        static void Usage()
        {
            Console.WriteLine("Rundll.exe: Run unmanaged DLLs in a new thread inside the current process");
            Console.WriteLine("            Optionally specify a DLL export function to call from the loaded DLL");
            Console.WriteLine("Usage:      Rundll.exe <filename|b64_dll_bytes> [method]");
            Console.WriteLine("Example:    Rundll.exe msf.dll");
            Console.WriteLine("            Rundll.exe badrat.dll Run");
            Console.WriteLine("            Rundll.exe nimplant.dll Update");
            Console.WriteLine("Note:       DllMain is always called when the DLL is loaded");
        }
        public static int Main(string[] args)
        {
            ...
            try
            {
                ...
                {
                    // input is a DLL from disk
                    data = File.ReadAllBytes(args[0]);
                }
                else
                {
                    data = Convert.FromBase64String(args[0]);
                }
            }
            catch (Exception e)
            {
                Console.WriteLine($"[-] Exception caught trying to parse/load DLL file/bytes: {e}");
                return 0;
            }
            if (args.Length > 1) // There is a DLL export to run
            {
                function = args[1];
            }
            ...
            byte[] userData = System.Text.Encoding.Default.GetBytes("\0"); // TODO: Allow arguments to be passed to the DLL export function
            var shellcode = ConvertToShellcode(data, HashFunction("DllMain"), userData, 0);
            ...
            if (function != null)
            {
                IntPtr expFunctionLocation = PE.GetProcAddressR(peLocation, function);
                if (expFunctionLocation != IntPtr.Zero)
                {
                    ...
                    Console.WriteLine($"[+] Calling exported function: {function}");
